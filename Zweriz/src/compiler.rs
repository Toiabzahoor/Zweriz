...
pub enum Opcode {
    ...
    CallClosure(usize, usize, usize, usize), NativeCall(usize, usize, usize, usize), Return(usize), 
    ...
    SetGpuMode(bool),
    AllocObject(usize, String),
    MethodCall { target_reg: usize, dest_reg: usize, arg_start: usize, arg_count: usize, method_name: String },
    RegisterMethod(String, String, usize),
    MakeClosure(usize, usize, usize, usize, usize, isize),
    SpawnThread { target_reg: usize, dest_reg: usize, arg_start: usize, arg_count: usize },
    JoinThread(usize, usize),
}
...
struct LoopContext { break_jmp_indices: Vec<usize>, continue_jmp_indices: Vec<usize> }
...
pub struct Compiler {
    ...
    pub classes: HashMap<String, Vec<(String, Expr)>>,
}

impl Compiler {
    pub fn new() -> Self {
        let mut native_functions = HashMap::new();
        native_functions.insert("random.float".to_string(), 0); native_functions.insert("random.uniform".to_string(), 1); 
        native_functions.insert("random.randint".to_string(), 2); native_functions.insert("io.read".to_string(), 3); 
        native_functions.insert("io.write".to_string(), 4); native_functions.insert("io.append".to_string(), 5); 
        native_functions.insert("io.exists".to_string(), 6); native_functions.insert("mmap.load_f64".to_string(), 7); 
        native_functions.insert("os.exit".to_string(), 8); native_functions.insert("os.env".to_string(), 9); 
        native_functions.insert("os.cmd".to_string(), 10); native_functions.insert("time.sleep".to_string(), 11); 
        native_functions.insert("time.now".to_string(), 12); native_functions.insert("math.floor".to_string(), 13); 
        native_functions.insert("math.ceil".to_string(), 14); native_functions.insert("math.round".to_string(), 15); 
        native_functions.insert("math.pow".to_string(), 16); native_functions.insert("string.len".to_string(), 17); 
        native_functions.insert("string.parse_num".to_string(), 18); native_functions.insert("random.normal".to_string(), 19); 
        native_functions.insert("math.transpose".to_string(), 20); native_functions.insert("math.trapz".to_string(), 21); 
        native_functions.insert("math.gradient".to_string(), 22); native_functions.insert("math.shift2d".to_string(), 23); 
        native_functions.insert("net.http_get".to_string(), 30); native_functions.insert("net.http_post".to_string(), 31); 
        native_functions.insert("net.tcp_send".to_string(), 32); native_functions.insert("net.tcp_listen".to_string(), 33); 
        native_functions.insert("chess_batch_generate".to_string(), 100); native_functions.insert("search".to_string(), 101); 
        native_functions.insert("nn.relu".to_string(), 40); native_functions.insert("nn.sigmoid".to_string(), 41); 
        native_functions.insert("nn.softmax".to_string(), 42); native_functions.insert("nn.track".to_string(), 43); 
        native_functions.insert("nn.backward".to_string(), 44); native_functions.insert("nn.step".to_string(), 45); 
        native_functions.insert("nn.zero_grad".to_string(), 46); native_functions.insert("nn.tanh".to_string(), 47); 
        native_functions.insert("nn.leaky_relu".to_string(), 48); native_functions.insert("nn.gelu".to_string(), 49); 
        native_functions.insert("nn.dropout".to_string(), 50); native_functions.insert("nn.swish".to_string(), 51); 
        native_functions.insert("nn.silu".to_string(), 51); native_functions.insert("nn.softplus".to_string(), 52); 
        native_functions.insert("io.delete".to_string(), 60); native_functions.insert("string.to_lower".to_string(), 61); 
        native_functions.insert("string.to_upper".to_string(), 62); native_functions.insert("string.contains".to_string(), 63); 
        native_functions.insert("array.shape".to_string(), 64); native_functions.insert("array.push".to_string(), 65); 
        native_functions.insert("array.pop".to_string(), 66); native_functions.insert("array.clone".to_string(), 67); 
        native_functions.insert("array.concat".to_string(), 68); native_functions.insert("math.popcount".to_string(), 69); 
        native_functions.insert("math.tzcnt".to_string(), 70); native_functions.insert("math.lzcnt".to_string(), 71); 
        native_functions.insert("math.min".to_string(), 72); native_functions.insert("math.max".to_string(), 73); 
        native_functions.insert("string.char_code_at".to_string(), 74); native_functions.insert("string.from_char_code".to_string(), 75);
        native_functions.insert("ffi.load".to_string(), 80); native_functions.insert("ffi.call".to_string(), 81);

        let mut comp = Self { 
            scopes: Vec::new(), bytecode: Vec::new(), 
            loop_contexts: Vec::new(), user_functions: HashMap::new(), 
            native_functions, classes: HashMap::new()
        }; 
    ...
    pub fn enter_block_scope(&mut self) { 
        let next_idx = self.scopes.last().map(|s| s.next_var_idx).unwrap_or(0); 
        self.scopes.push(Scope { environment: HashMap::new(), next_var_idx: next_idx, is_function: false, captures: Vec::new() }); 
    }
    
    pub fn enter_func_scope(&mut self) { 
        self.scopes.push(Scope { environment: HashMap::new(), next_var_idx: 0, is_function: true, captures: Vec::new() }); 
    }
    
    pub fn exit_scope(&mut self) { self.scopes.pop(); }
    
    fn find_and_capture(&mut self, name: &str, scope_idx: usize) -> Option<usize> {
        if let Some(&idx) = self.scopes[scope_idx].environment.get(name) {
            return Some(idx);
        }
        if scope_idx == 0 { return None; }
        
        if self.scopes[scope_idx].is_function {
            if let Some(_parent_idx) = self.find_and_capture(name, scope_idx - 1) {
                let idx = self.scopes[scope_idx].next_var_idx;
                if idx >= 256 { panic!("Compiler Error: Register Limit Exceeded."); }
                self.scopes[scope_idx].environment.insert(name.to_string(), idx);
                self.scopes[scope_idx].next_var_idx += 1;
                self.scopes[scope_idx].captures.push(name.to_string());
                return Some(idx);
            }
        } else {
            if let Some(idx) = self.find_and_capture(name, scope_idx - 1) {
                return Some(idx);
            }
        }
        None
    ...
    fn resolve_var(&mut self, name: &str) -> Result<usize, String> { 
        if let Some(idx) = self.find_and_capture(name, self.scopes.len() - 1) { 
            return Ok(idx); 
        } 
        let current = self.scopes.last_mut().unwrap(); 
        let idx = current.next_var_idx; 
        if idx >= 256 { return Err("Compiler Error: Register Limit Exceeded.".to_string()); } 
        current.environment.insert(name.to_string(), idx); 
        current.next_var_idx += 1; 
        Ok(idx) 
    }
    
    pub fn has_var(&mut self, name: &str) -> bool { self.find_and_capture(name, self.scopes.len() - 1).is_some() }
    
    pub fn compile(&mut self, ast: Vec<Statement>) -> Result<Vec<Opcode>, String> {
        for stmt in ast { self.compile_statement(stmt)?; } 
    ...
    fn is_gpu_compatible_expr(&self, expr: &Expr) -> bool {
        match expr { 
            ...
            Expr::MatrixOp { .. } => false, Expr::UnaryOp { right, .. } => self.is_gpu_compatible_expr(right), 
    ...
    fn is_gpu_compatible_stmt(&self, stmt: &Statement) -> bool {
        match stmt { 
            ...
            Statement::ForEach { .. } => false, Statement::Return { value } => self.is_gpu_compatible_expr(value), Statement::Break | Statement::Continue => true, 
            _ => false, 
        }
    }
    ...
    fn transpile_expr(&mut self, expr: &Expr, inputs: &mut Vec<String>, outputs: &mut Vec<String>, scalars: &mut Vec<String>, req_funcs: &mut Vec<String>) -> Result<String, String> {
        match expr { 
            ...
            Expr::FunctionCall { name, args } => { 
                ...
                if math_funcs.contains(&name.as_str()) { let c_name = name.replace("math.", ""); return Ok(format!("{}({})", c_name, arg_strs.join(", "))); } 
                if name == "blend" { return Ok(format!("(({}) != 0.0 ? ({}) : ({}))", arg_strs[0], arg_strs[1], arg_strs[2])); } 
                if self.user_functions.contains_key(name) { if !req_funcs.contains(name) { req_funcs.push(name.clone()); } return Ok(format!("zweriz_usr_{}({})", name, arg_strs.join(", "))); } 
    ...
    fn transpile_stmt(&mut self, stmt: &Statement, inputs: &mut Vec<String>, outputs: &mut Vec<String>, scalars: &mut Vec<String>, req_funcs: &mut Vec<String>) -> Result<String, String> {
        match stmt { 
    ...
    pub fn compile_statement(&mut self, stmt: Statement) -> Result<(), String> {
        ...
        match stmt {
            ...
            Statement::Continue => { let jmp_idx = self.bytecode.len(); self.bytecode.push(Opcode::Jmp(0)); self.loop_contexts.last_mut().unwrap().continue_jmp_indices.push(jmp_idx); }
            ...
            Statement::For { iterator, start, end, body } => {
                ...
                let loop_start = self.bytecode.len(); self.loop_contexts.push(LoopContext { break_jmp_indices: Vec::new(), continue_jmp_indices: Vec::new() });
                ...
                let continue_pc = self.bytecode.len();
                ...
                let end_pc = self.bytecode.len(); self.bytecode[jmp_false_idx] = Opcode::JmpIfFalse(cond_reg, end_pc); 
                let ctx = self.loop_contexts.pop().unwrap(); 
                for break_idx in ctx.break_jmp_indices { self.bytecode[break_idx] = Opcode::Jmp(end_pc); } 
                for cont_idx in ctx.continue_jmp_indices { self.bytecode[cont_idx] = Opcode::Jmp(continue_pc); }
                self.exit_scope();
            }
            Statement::ForEach { iterator, array, body } => {
                ...
                self.bytecode.push(Opcode::LoadElement(iter_reg, arr_reg, idx_reg)); 
            ...
            Statement::ClassDecl { name, properties, methods } => {
                self.classes.insert(name.clone(), properties);
                for method in methods {
                    if let Statement::FunctionDecl { name: method_name, params, body } = method {
                        let jmp_over_idx = self.bytecode.len();
                        self.bytecode.push(Opcode::Jmp(0));
                        let func_pc = self.bytecode.len();
                        self.enter_func_scope();
                        self.resolve_var("self")?; 
                        for param in params { self.resolve_var(&param)?; }
                        for s in body { self.compile_statement(s)?; }
                        self.bytecode.push(Opcode::LoadConst(255, 0.0));
                        self.bytecode.push(Opcode::Return(255));
                        self.exit_scope();
                        self.bytecode[jmp_over_idx] = Opcode::Jmp(self.bytecode.len());
                        self.bytecode.push(Opcode::RegisterMethod(name.clone(), method_name, func_pc));
                    }
                }
            }
            Statement::FunctionDecl { name, params, body } => { 
                self.user_functions.insert(name.clone(), (params.clone(), body.clone())); 
                let dest_reg = self.resolve_var(&name)?; 
                ...
                self.bytecode.push(Opcode::LoadConst(255, 0.0)); 
                self.bytecode.push(Opcode::Return(255)); 
                let captures = self.scopes.pop().unwrap().captures; 
                
                self.bytecode[jmp_over_idx] = Opcode::Jmp(self.bytecode.len()); 
                
                let current_temp = self.scopes.last().unwrap().next_var_idx;
                let capture_start = current_temp;
                let mut self_capture_idx = -1;
                for (i, cap_name) in captures.iter().enumerate() {
                    let src_reg = self.find_and_capture(cap_name, self.scopes.len() - 1).unwrap();
                    if src_reg == dest_reg { self_capture_idx = i as isize; }
                    self.bytecode.push(Opcode::Move(capture_start + i, src_reg));
                }
                
                self.bytecode.push(Opcode::MakeClosure(dest_reg, func_pc, capture_start, captures.len(), params.len(), self_capture_idx));
            }
            ...
            Statement::While { condition, body } => { let loop_start = self.bytecode.len(); self.loop_contexts.push(LoopContext { break_jmp_indices: Vec::new(), continue_jmp_indices: Vec::new() }); let cond_reg = current_temp; self.compile_expression(condition, cond_reg, current_temp + 1)?; let jmp_false_idx = self.bytecode.len(); self.bytecode.push(Opcode::JmpIfFalse(cond_reg, 0)); for s in body { self.compile_statement(s)?; } self.bytecode.push(Opcode::Jmp(loop_start)); let end_pc = self.bytecode.len(); self.bytecode[jmp_false_idx] = Opcode::JmpIfFalse(cond_reg, end_pc); let ctx = self.loop_contexts.pop().unwrap(); for break_idx in ctx.break_jmp_indices { self.bytecode[break_idx] = Opcode::Jmp(end_pc); } for cont_idx in ctx.continue_jmp_indices { self.bytecode[cont_idx] = Opcode::Jmp(loop_start); } }
            Statement::GpuBlock { statements } => { 
                self.bytecode.push(Opcode::SetGpuMode(true)); self.compile_gpu_block(&statements)?; self.bytecode.push(Opcode::SetGpuMode(false)); 
            } 
    ...
    fn compile_expression(&mut self, expr: Expr, dest: usize, current_temp: usize) -> Result<(), String> {
        match expr {
            Expr::Number(val) => self.bytecode.push(Opcode::LoadConst(dest, val)), Expr::Boolean(val) => self.bytecode.push(Opcode::LoadConst(dest, if val { 1.0 } else { 0.0 })), Expr::StringLiteral(text) => self.bytecode.push(Opcode::LoadString(dest, text)), 
            Expr::Identifier(name) => { let idx = self.resolve_var(&name)?; if idx != dest { self.bytecode.push(Opcode::Move(dest, idx)); } }
            ...
            Expr::FunctionCall { name, args } => {
                if name == "thread.spawn" {
                    if args.is_empty() { return Err("thread.spawn requires at least 1 argument (the function)".to_string()); }
                    let target_reg = current_temp;
                    self.compile_expression(args[0].clone(), target_reg, current_temp + 1)?;
                    let arg_count = args.len() - 1;
                    let arg_start = current_temp + 1;
                    for (i, arg) in args.into_iter().skip(1).enumerate() {
                        self.compile_expression(arg, arg_start + i, arg_start + arg_count)?;
                    }
                    self.bytecode.push(Opcode::SpawnThread { target_reg, dest_reg: dest, arg_start, arg_count });
                    return Ok(());
                }
                if name == "thread.join" {
                    if args.len() != 1 { return Err("thread.join requires exactly 1 argument (the thread id)".to_string()); }
                    self.compile_expression(args[0].clone(), current_temp, current_temp + 1)?;
                    self.bytecode.push(Opcode::JoinThread(dest, current_temp));
                    return Ok(());
                }
                if let Some(props) = self.classes.get(&name).cloned() {
                    let obj_reg = current_temp;
                    self.bytecode.push(Opcode::AllocObject(obj_reg, name.clone()));
                    for (prop_name, expr) in props {
                        let val_reg = current_temp + 1;
                        self.compile_expression(expr, val_reg, current_temp + 2)?;
                        let prop_name_reg = current_temp + 2;
                        self.bytecode.push(Opcode::LoadString(prop_name_reg, prop_name));
                        self.bytecode.push(Opcode::StoreElement(obj_reg, prop_name_reg, val_reg));
                    }
                    if dest != obj_reg {
                        self.bytecode.push(Opcode::Move(dest, obj_reg));
                    }
                    return Ok(());
                }
                ...
                if name == "sum" || name == "mean" || name == "max" || name == "min" { 
                    ...
                    if args.len() == 2 { self.compile_expression(args[1].clone(), current_temp + 1, current_temp + 2)?; } else { self.bytecode.push(Opcode::LoadConst(current_temp + 1, -1.0)); } 
                    self.bytecode.push(Opcode::Reduce(dest, current_temp, current_temp + 1, op_type)); return Ok(()); 
                }
                ...
                let target_reg = current_temp;
                self.compile_expression(Expr::Identifier(name.clone()), target_reg, current_temp + 1)?;
                ...
                self.bytecode.push(Opcode::CallClosure(target_reg, dest, arg_start, arg_count));
            }
            Expr::MethodCall { target, method, args } => {
                if let Expr::Identifier(ref obj_name) = *target {
                    let combined = format!("{}.{}", obj_name, method);
                    // Add explicitly checks for "thread.spawn" and "thread.join" right here
                    if combined == "thread.spawn" || combined == "thread.join" || self.native_functions.contains_key(&combined) || self.user_functions.contains_key(&combined) {
                        return self.compile_expression(Expr::FunctionCall { name: combined, args: args.clone() }, dest, current_temp);
                    }
                }
                                 
                let target_reg = current_temp;
                self.compile_expression(*target, target_reg, current_temp + 1)?;
                let arg_count = args.len();
                let arg_start = current_temp + 1;
                for (i, arg) in args.into_iter().enumerate() {
                    self.compile_expression(arg, arg_start + i, arg_start + arg_count)?;
                }
                self.bytecode.push(Opcode::MethodCall { 
                    target_reg, dest_reg: dest, arg_start, arg_count, method_name: method 
                });
            }
